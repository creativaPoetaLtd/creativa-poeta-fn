...
const initialManualEntry: ManualReferralEntryPayload = {
  ...
  companyName: "", contactName: "", contactEmail: "", contactPhone: "", clientWebsite: "",
  serviceNeeded: "", budgetRange: "", needDescription: "", relationship: "",
  consentStatus: "not_yet", introductionMethod: "WhatsApp", introductionDetails: "",
};
    ...
    <DialogContent dividers sx={{ p: { xs: 2, sm: 3 } }}>
      ...
      {result ? <Box sx={{ display: "grid", gap: 2 }}>
        ...
        {form.includeClient && <><Divider /><Typography variant="h6" fontWeight={900}>Introduced client</Typography><Grid container spacing={2}>
          <Grid item xs={12} sm={6}><TextField required fullWidth label="Company" value={form.companyName} onChange={(event) => setForm({ ...form, companyName: event.target.value })} /></Grid>
          <Grid item xs={12} sm={6}><TextField required fullWidth label="Contact person" value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })} /></Grid>
          ...
          <Grid item xs={12} sm={6}><TextField fullWidth label="Company website (optional)" value={form.clientWebsite} onChange={(event) => setForm({ ...form, clientWebsite: event.target.value })} /></Grid>
          ...
          <Grid item xs={12}><TextField required fullWidth multiline minRows={3} label="Client need" value={form.needDescription} onChange={(event) => setForm({ ...form, needDescription: event.target.value })} /></Grid>
          ...
          <Grid item xs={12} sm={6}><TextField fullWidth label="Introduction channel" value={form.introductionMethod} onChange={(event) => setForm({ ...form, introductionMethod: event.target.value })} /></Grid>
          <Grid item xs={12}><TextField fullWidth multiline minRows={2} label="Context / notes" value={form.introductionDetails} onChange={(event) => setForm({ ...form, introductionDetails: event.target.value })} /></Grid>
        </Grid></>}
      </Box>}
    </DialogContent>
