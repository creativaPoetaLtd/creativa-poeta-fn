...
export const submitCareerApplication = async (data: CareerApplicationPayload) => {
  const response = await publicRequest<{ message: string; applicationId?: string }>({ method: "POST", url: "/api/job/apply", data }, "Unable to send your application.");
