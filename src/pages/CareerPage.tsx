...
const controlClass = "min-h-12 w-full rounded-xl border border-white/20 bg-transparent px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-slate-500 focus:border-[#EEBA2B] focus:bg-transparent focus:ring-2 focus:ring-[#EEBA2B]/20";
      ...
      <section id="career-application" className="scroll-mt-24 border-y border-white/10 bg-black/10 py-10 backdrop-blur-[1px] sm:py-14">
        <div className="mx-auto grid max-w-6xl items-start gap-7 px-4 md:px-8 lg:grid-cols-[minmax(15rem,.7fr)_minmax(0,1.3fr)]">
          ...
          <form noValidate onSubmit={submit} encType="multipart/form-data" className="grid gap-4 rounded-2xl border border-white/15 bg-black/20 p-4 backdrop-blur-md sm:grid-cols-2 sm:p-6">
            ...
            <FormField label={copy.labels.discoverySource} required wide><select required aria-required="true" className={`${controlClass} border-red-500/90 focus:border-red-400 focus:ring-red-500/20`} value={application.discoverySource} onChange={(event) => setApplication({ ...application, discoverySource: event.target.value as CareerDiscoverySource, discoverySourceOther: event.target.value === "other" ? application.discoverySourceOther : "" })}><option className="bg-slate-950" value="" disabled>—</option>{copy.discoveryOptions.map((option) => <option className="bg-slate-950" key={option.value} value={option.value}>{option.label}</option>)}</select></FormField>
            {application.discoverySource === "other" && <FormField label={copy.labels.discoveryOther} required wide><input required className={controlClass} value={application.discoverySourceOther} onChange={(event) => setApplication({ ...application, discoverySourceOther: event.target.value })} /></FormField>}
            <div className="sm:col-span-2"><FieldLabel label={copy.labels.cv} /><label htmlFor="career-cv" className="flex min-h-20 cursor-pointer items-center gap-3 rounded-xl border border-dashed border-white/25 bg-transparent px-4 py-3 transition hover:border-[#EEBA2B]"><span className="rounded-full bg-[#EEBA2B]/15 p-3 text-[#ffee00]"><FaFileUpload /></span><span className="min-w-0"><span className="block truncate text-sm font-black text-white">{cvFile ? `${copy.cvSelected}: ${cvFile.name}` : copy.labels.cv}</span><span className="mt-1 block text-xs font-semibold text-slate-400">{copy.cvHint}</span></span></label><input id="career-cv" type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" className="sr-only" onChange={(event) => selectCv(event.target.files?.[0])} /></div>
            ...
            <label className="flex items-start gap-3 text-sm font-semibold leading-relaxed text-slate-300 sm:col-span-2"><input required type="checkbox" className="mt-1 h-5 w-5 min-h-5 min-w-5 shrink-0 aspect-square accent-[#EEBA2B]" checked={application.consentAccepted} onChange={(event) => setApplication({ ...application, consentAccepted: event.target.checked })} /><span>{copy.consent}<RequiredMark /></span></label>
...
function FormField({ label, wide = false, required = false, children }: { label: string; wide?: boolean; required?: boolean; children: ReactNode }) {
  return <label className={`block min-w-0 ${wide ? "sm:col-span-2" : ""}`}><FieldLabel label={label} required={required} />{children}</label>;
}

function FieldLabel({ label, required = false }: { label: string; required?: boolean }) {
  return <span className={labelClass}>{label}{required && <RequiredMark />}</span>;
}
