...
import { FaArrowRight, FaBriefcase, FaCheck, FaClock, FaGlobeEurope, FaHandshake, FaLaptop, FaMapMarkerAlt, FaPaperPlane, FaTimes } from "react-icons/fa";
...
const controlClass = "min-h-12 w-full rounded-xl border border-white/20 bg-black/30 px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-slate-500 focus:border-[#EEBA2B] focus:ring-2 focus:ring-[#EEBA2B]/20";
const labelClass = "mb-2 block text-[11px] font-black uppercase tracking-[.06em] text-slate-300 sm:text-xs";
const buttonClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-full px-5 text-center text-xs font-black transition sm:min-h-12 sm:px-6 sm:text-sm";

const initialApplication = {
  fullName: "", email: "", phone: "", country: "", desiredRole: "", experience: "", skills: "",
  linkedin: "", portfolio: "", availability: "", message: "", consentAccepted: false, websiteConfirmation: "",
};
...
export default function CareerPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      setSending(true);
      await submitCareerApplication({ ...application, jobId: selectedJob?._id, locale });
  ...
  return <PageLayout className="bg-[#071426] text-white">
    ...
    <main>
      <section className="relative overflow-hidden border-b border-white/10 pt-28 sm:pt-36">
        <div className="absolute inset-0 bg-cover bg-center opacity-30" style={{ backgroundImage: `url(${backImage})` }} />
        <div className="absolute inset-0 bg-[linear-gradient(100deg,rgba(1,10,23,.98)_10%,rgba(7,26,51,.86)_58%,rgba(7,26,51,.72))]" />
        <div className="relative mx-auto grid max-w-6xl items-end gap-8 px-4 pb-12 md:px-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(18rem,.75fr)] lg:pb-16">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <a href="#career-opportunities" className={`${buttonClass} bg-[#ffee00] text-black hover:bg-white`}>{copy.primaryCta}<FaArrowRight /></a>
              <button type="button" onClick={() => chooseJob(null)} className={`${buttonClass} border border-white/35 bg-white/5 text-white hover:border-[#EEBA2B] hover:text-[#ffee00]`}>{copy.secondaryCta}</button>
            </div>
          </div>
      ...
      <section id="career-opportunities" className="scroll-mt-24 py-10 sm:py-14">
        <div className="mx-auto max-w-6xl px-4 md:px-8">
          ...
          <div className="grid items-start gap-4 lg:grid-cols-2">
            <article className="rounded-2xl border border-[#EEBA2B]/45 bg-[linear-gradient(145deg,rgba(238,186,43,.16),rgba(7,26,51,.72))] p-5 backdrop-blur-md sm:p-7">
            ...
            {loading ? <div className="min-h-64 animate-pulse rounded-2xl border border-white/10 bg-white/[.04]" /> : jobs.length === 0 ? (
              <article className="rounded-2xl border border-white/15 bg-black/25 p-5 backdrop-blur-md sm:p-7">
            ...
            ) : <div className="grid gap-3">{jobs.map((job) => <article key={job._id} className="rounded-2xl border border-white/15 bg-black/25 p-5 backdrop-blur-md transition hover:border-[#EEBA2B]/60">
      ...
      <section id="career-application" className="scroll-mt-24 border-y border-white/10 bg-black/20 py-10 sm:py-14">
        <div className="mx-auto grid max-w-6xl items-start gap-7 px-4 md:px-8 lg:grid-cols-[minmax(15rem,.7fr)_minmax(0,1.3fr)]">
          ...
          <form onSubmit={submit} className="grid gap-4 rounded-2xl border border-white/15 bg-[#071426]/75 p-4 backdrop-blur-md sm:grid-cols-2 sm:p-6">
            ...
            <FormField label={copy.labels.desiredRole}><input required={!selectedJob} className={controlClass} value={application.desiredRole} onChange={(event) => setApplication({ ...application, desiredRole: event.target.value })} /></FormField>
            <FormField label={copy.labels.experience}><input className={controlClass} value={application.experience} onChange={(event) => setApplication({ ...application, experience: event.target.value })} /></FormField>
            ...
            <FormField label={copy.labels.linkedin}><input type="url" className={controlClass} value={application.linkedin} onChange={(event) => setApplication({ ...application, linkedin: event.target.value })} /></FormField>
            <FormField label={copy.labels.portfolio}><input type="url" className={controlClass} value={application.portfolio} onChange={(event) => setApplication({ ...application, portfolio: event.target.value })} /></FormField>
            ...
            <FormField label={copy.labels.message} wide><textarea rows={4} className={controlClass} value={application.message} onChange={(event) => setApplication({ ...application, message: event.target.value })} /></FormField>
            ...
            <div className="flex justify-start sm:col-span-2"><button disabled={sending} className={`${buttonClass} bg-[#ffee00] text-black hover:bg-white disabled:opacity-60`}>{sending ? copy.submitting : copy.submit}<FaPaperPlane /></button></div>
          </form>
        </div>
      </section>
    </main>
