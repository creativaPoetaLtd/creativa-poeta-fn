...
const inputClass = "min-h-[46px] w-full rounded-lg border border-white/20 bg-black/30 px-3.5 py-2.5 text-sm font-semibold text-white outline-none transition placeholder:text-slate-500 focus:border-[#EEBA2B] focus:ring-2 focus:ring-[#EEBA2B]/20";
const labelClass = "mb-1.5 block text-xs font-black uppercase tracking-wide text-slate-300";
...
export default function ReferralProgramPage() {
  ...
  return <PageLayout className="text-white">
    ...
    <main className="min-h-screen overflow-hidden bg-[linear-gradient(180deg,rgba(5,12,22,.3),rgba(5,12,22,.74))] pt-24 sm:pt-28">
      <section className="mx-auto grid w-full max-w-7xl gap-7 px-4 pb-9 pt-4 md:px-8 lg:min-h-[68vh] lg:grid-cols-[1.2fr_.8fr] lg:items-center lg:pb-14">
        <div>
          ...
          <div className="mt-6 flex flex-col gap-2.5 sm:flex-row">
            <a href="#join-cprpp" className="inline-flex min-h-[46px] items-center justify-center gap-2 rounded-full bg-[#ffee00] px-6 text-sm font-black text-black hover:bg-white">{copy.join}<FaArrowRight /></a>
            <a href="#submit-referral" className="inline-flex min-h-[46px] items-center justify-center gap-2 rounded-full border border-white/30 bg-black/20 px-6 text-sm font-black backdrop-blur hover:border-[#EEBA2B]">{copy.submitLead}</a>
          </div>
      ...
      <section className="border-y border-white/10 bg-black/20 py-9 backdrop-blur-sm sm:py-14"><div className="mx-auto max-w-7xl px-4 md:px-8">
      ...
      <section className="mx-auto max-w-7xl px-4 py-9 md:px-8 sm:py-14"><h2 className="text-2xl font-black sm:text-4xl">{copy.servicesTitle}</h2><div className="mt-5 flex flex-wrap gap-2">{copy.services.map((service) => <span key={service} className="rounded-full border border-[#EEBA2B]/35 bg-black/25 px-3.5 py-2 text-xs font-black sm:text-sm">{service}</span>)}</div></section>

      <section id="business-partners" className="mx-auto grid max-w-7xl scroll-mt-28 gap-4 px-4 pb-9 md:px-8 lg:grid-cols-2 sm:pb-14">
        <article className="rounded-2xl border border-[#EEBA2B]/30 bg-black/25 p-5 backdrop-blur sm:p-7"><FaCalculator className="text-2xl text-[#EEBA2B]" /><h2 className="mt-3 text-2xl font-black sm:text-3xl">{copy.calculatorTitle}</h2><p className="mt-2 text-sm font-semibold leading-relaxed text-slate-300">{copy.calculatorLead}</p><label className="mt-5 block"><span className={labelClass}>{copy.estimatedValue}</span><div className="flex items-center rounded-lg border border-white/20 bg-black/30 px-3"><span className="font-black text-[#EEBA2B]">€</span><input type="number" min="0" step="50" value={projectValue} onChange={(event) => setProjectValue(Number(event.target.value))} className="min-h-[48px] w-full bg-transparent px-2 font-black outline-none" /></div></label><div className="mt-4 rounded-xl bg-[#EEBA2B] p-4 text-black"><p className="text-xs font-black uppercase">{copy.estimatedReward}</p><p className="mt-1 text-4xl font-black">€{estimatedReward.toFixed(2)}</p></div><p className="mt-3 text-xs font-semibold leading-relaxed text-slate-400">{copy.estimateNote}</p></article>
        ...
        ].map(({ title, text, points, icon: Icon }) => <article key={title} className="rounded-xl border border-white/15 bg-black/25 p-4 backdrop-blur"><Icon className="text-xl text-[#EEBA2B]" /><h3 className="mt-3 text-xl font-black">{title}</h3><p className="mt-2 text-sm font-semibold text-slate-300">{text}</p><ul className="mt-3 space-y-2">{points.map((point) => <li key={point} className="flex gap-2 text-xs font-bold text-slate-200"><FaCheck className="mt-0.5 shrink-0 text-[#EEBA2B]" />{point}</li>)}</ul></article>)}</div></div>
      </section>

      <section className="border-y border-white/10 bg-black/20 py-9 sm:py-14"><div className="mx-auto grid max-w-7xl gap-4 px-4 md:grid-cols-2 md:px-8"><article><h2 className="text-2xl font-black text-emerald-300">{copy.validTitle}</h2><ul className="mt-4 grid gap-2">{copy.valid.map((item) => <li key={item} className="flex gap-2 rounded-lg bg-emerald-500/10 p-3 text-sm font-bold"><FaCheck className="mt-0.5 text-emerald-300" />{item}</li>)}</ul></article><article><h2 className="text-2xl font-black text-red-300">{copy.invalidTitle}</h2><ul className="mt-4 grid gap-2">{copy.invalid.map((item) => <li key={item} className="flex gap-2 rounded-lg bg-red-500/10 p-3 text-sm font-bold"><FaTimes className="mt-0.5 text-red-300" />{item}</li>)}</ul></article></div></section>

      {referralCode && <section id="referred-business" className="mx-auto max-w-5xl scroll-mt-28 px-4 py-9 md:px-8 sm:py-14"><div className="rounded-2xl border border-emerald-400/35 bg-emerald-500/10 p-4 sm:p-7"><p className="text-xs font-black uppercase tracking-wider text-emerald-300">Verified referral link</p><h2 className="mt-2 text-3xl font-black sm:text-4xl">{prospectCopy.title}</h2><p className="mt-2 max-w-3xl text-sm font-semibold text-slate-200 sm:text-base">{prospectCopy.lead}</p>
        <form data-analytics-form="prospect_confirmed_referral" onSubmit={handleProspect} className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="flex gap-2 text-sm font-bold sm:col-span-2"><input required type="checkbox" checked={prospect.contactConsent} onChange={(event) => setProspect({ ...prospect, contactConsent: event.target.checked })} className="mt-1 h-4 w-4 accent-[#EEBA2B]" />{prospectCopy.consent}</label>
          <SubmitButton sending={prospectSending} label={prospectCopy.send} sendingLabel={copy.form.sending} />
        </form>
      </div></section>}

      <section id="join-cprpp" className="mx-auto max-w-5xl scroll-mt-28 px-4 py-9 md:px-8 sm:py-14"><h2 className="text-3xl font-black sm:text-4xl">{copy.applyTitle}</h2><p className="mt-2 max-w-3xl text-sm font-semibold text-slate-300 sm:text-base">{copy.applyLead}</p>
        <form data-analytics-form="referral_partner_application" onSubmit={handleApplication} className="mt-5 grid gap-4 rounded-2xl border border-white/15 bg-black/30 p-4 backdrop-blur sm:grid-cols-2 sm:p-7">
          ...
          <label className="flex gap-2 text-sm font-semibold text-slate-300 sm:col-span-2"><input type="checkbox" checked={application.marketingConsent} onChange={(event) => setApplication({ ...application, marketingConsent: event.target.checked })} className="mt-1 h-4 w-4 accent-[#EEBA2B]" />{copy.form.marketing}</label>
          <SubmitButton sending={applicationSending} label={copy.form.send} sendingLabel={copy.form.sending} />
        </form>
      </section>

      <section id="submit-referral" className="scroll-mt-28 border-y border-white/10 bg-black/20 py-9 sm:py-14"><div className="mx-auto max-w-5xl px-4 md:px-8"><h2 className="text-3xl font-black sm:text-4xl">{copy.leadTitle}</h2><p className="mt-2 max-w-3xl text-sm font-semibold text-slate-300 sm:text-base">{copy.leadLead}</p>
        {!partnerAccess.accessSecret ? <div className="mt-5 rounded-xl border border-[#EEBA2B]/30 bg-[#EEBA2B]/10 p-4 text-sm font-bold"><FaShieldAlt className="mb-2 text-xl text-[#EEBA2B]" />{copy.leadLocked}</div> : <form data-analytics-form="referral_lead" onSubmit={handleLead} className="mt-5 grid gap-4 rounded-2xl border border-[#EEBA2B]/30 bg-black/30 p-4 sm:grid-cols-2 sm:p-7">
