  ...
  return (
    <section id='services' className=' mainn py-16 justify-center  h-fit min-h-screen mt-0 flex flex-col bg-slate-800 items-center relative'>
      ...
      <div className="mt-[2%] pb-12   w-full flex felx-col flex-col space-y-7">
        ...
        <div className="watermark absolute laptop:h-fit desktop:h-fit w-full tablet:w-[20rem] h-fit laptop:w-[14rem] rounded-sm desktop:w-[14rem] bg-blue-900/20 font-extrabold border-l-4 border-r-4 border-b-4 border-t-4 border-[#3B1983] opacity-75 laptop:right-[1rem] tablet:right-[2rem] desktop:right-[1rem] laptop:top-[50%] desktop:top-[50%] tablet:top-[50%] phone:top-[69rem] phone:w-[72%] phone:right-2 xs:top-[75rem] xs:w-[14.5rem] sumsung:top-[66.4rem] iphone:top-[66.5rem] xs:right-1 px-1"
          style={{
            boxShadow: "inset 0 0 20px rgba(0, 0, 255, 0.5)", // Blue glow inside
          }}
        >
          <p className="text-[#FF676A] text-xs" style={{ textShadow: "0 0 3px rgb(245, 98, 122)" }}>
            SPECIALEMENT POUR YOUTUBEUR, TIKTOK, INSTAGRAM, ....
          </p>
          <p className="text-[#FF676A] text-xs" style={{ textShadow: "0 0 3px rgb(245, 98, 122)" }}>
            ETU/OU CREATEUR DE CONTENU
          </p>
        </div>
      </div>
